...
using Inventory.UwpApp.Library.Tools;
using Inventory.UwpApp.Library.Extensions;
...
namespace Inventory.UwpApp.Library.Controls
{
    public sealed partial class DataList : UserControl, INotifyExpressionChanged
    {
        ...
        public bool IsDataAvailable
        {
            get
            {
                //var v = ItemsSource?.Cast<object>().Any() ?? false;
                var v = ((ICollection)ItemsSource)?.Count != 0;
                return v;
                //return true;
            }
        }
